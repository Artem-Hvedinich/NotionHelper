...
export function formatMainMenu(): string {
  return `👋 *Добро пожаловать в NotionHelper!*

${createSeparator(25)}

🤖 *Что я умею:*

• 📝 Управлять задачами в Notion
• ☀️ Отслеживать утреннюю рутину
• 🕒 Работать с дневными задачами
• 📋 Вести список "Позже"
• 🌙 Контролировать вечерние дела
...
💡 *Как начать:*
Используй кнопки меню ниже для навигации. Каждая кнопка открывает соответствующий раздел с задачами из твоей базы Notion.

${createSeparator(25)}

🚀 *Готов начать? Выбери раздел из меню!*`;
}
